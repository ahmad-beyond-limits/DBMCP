...
export default function AdminDashboardPage() {
  ...
  const [compassModeActive, setCompassModeActive] = useState<boolean>(true);
  const [togglingCompassMode, setTogglingCompassMode] = useState<boolean>(false);
  ...
  const loadAdminData = async () => {
    try {
      ...
      const [statsData, usersData, workspacesData, guidanceData, globalRulesData, feedbackData, instructionDocsData] = await Promise.all([
        ...
        api.getAdminGlobalAIRules().catch(() => ({ id: 1, rules_text: "", compass_mode_active: true } as AIGlobalRules)),
      ...
      if (globalRulesData) {
        ...
        const activeFlag = globalRulesData.compass_mode_active !== undefined
          ? globalRulesData.compass_mode_active
          : globalRulesData.compas_mode_active;
        if (activeFlag !== undefined) {
          setCompassModeActive(activeFlag);
        }
      }
    } catch (err: any) {
  ...
  const handleToggleCompassMode = async () => {
    setTogglingCompassMode(true);
    const newStatus = !compassModeActive;
    // Optimistically update UI so the button toggles instantly
    setCompassModeActive(newStatus);
    try {
      const res = await api.toggleAdminCompassMode(newStatus);
      const activeFlag = res?.compass_mode_active !== undefined
        ? res.compass_mode_active
        : res?.compas_mode_active;
      if (activeFlag !== undefined) {
        setCompassModeActive(activeFlag);
      }
      setActionMsg({
        type: "success",
        text: `Compass Character & Security Shield is now ${newStatus ? "ACTIVE & ENFORCED" : "PAUSED"}.`,
      });
      setTimeout(() => {
        setActionMsg((curr) => curr?.text.includes("Compass Character") ? null : curr);
      }, 3500);
    } catch (err: any) {
      // Revert state if server update failed
      setCompassModeActive(!newStatus);
      let errMsg = "Failed to toggle Compass mode.";
  ...
  const handleSaveGlobalRules = async () => {
    ...
    try {
      await api.updateAdminGlobalAIRules({
        ...
        compass_mode_active: compassModeActive,
      });
  ...
  const handleUploadInstructionDoc = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      setInstructionDocProgress(`Uploading and securing document "${file.name}" for direct AI inspection...`);
      const doc = await api.uploadAdminGlobalInstructionDoc(file);
      setInstructionDocs((prev) => [doc, ...prev]);
      const successText = `Instruction document "${file.name}" uploaded successfully (${(doc.file_size / 1024).toFixed(1)} KB). AI will read and enforce this document directly.`;
        ...
        return (
          <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
            ...
            >
              ...
              {/* Compass Character & Anti-Hijacking Shield (Toggleable Master Directives) */}
              <div
                style={{
                  padding: "1rem 1.25rem",
                  borderRadius: "var(--radius-md)",
                  background: compassModeActive
                    ? "linear-gradient(135deg, rgba(249, 115, 22, 0.06) 0%, rgba(251, 146, 60, 0.02) 100%)"
                    : "rgba(0, 0, 0, 0.02)",
                  border: compassModeActive
              ...
              >
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "1rem", flexWrap: "wrap" }}>
                  <div style={{ display: "flex", alignItems: "flex-start", gap: "0.75rem", flex: 1, minWidth: "260px" }}>
                    <div
                      style={{
                        ...
                        height: "36px",
                        borderRadius: "10px",
                        background: compassModeActive ? "rgba(234, 88, 12, 0.12)" : "rgba(0, 0, 0, 0.05)",
                        color: compassModeActive ? "#C2410C" : "var(--text-tertiary)",
                        display: "flex",
                        alignItems: "center",
                    ...
                    <div>
                      <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                        <h4 style={{ margin: 0, fontSize: "0.95rem", fontWeight: 600, color: "var(--text-primary)", letterSpacing: "-0.01em" }}>
                          Compass Character &amp; Confidentiality Shield
                        </h4>
                        <span
                          style={{
                            fontSize: "0.68rem",
                            ...
                            borderRadius: "999px",
                            background: compassModeActive ? "rgba(22, 163, 74, 0.12)" : "rgba(0, 0, 0, 0.06)",
                            color: compassModeActive ? "#16A34A" : "var(--text-tertiary)",
                            border: compassModeActive ? "1px solid rgba(22, 163, 74, 0.25)" : "1px solid rgba(0, 0, 0, 0.1)",
                          }}
                        >
                          {compassModeActive ? "Enforced" : "Paused"}
                        </span>
                      </div>
                      <p style={{ margin: "0.2rem 0 0 0", fontSize: "0.8rem", color: "var(--text-secondary)", lineHeight: 1.45 }}>
                        Applies the official <strong>Compass</strong> persona: answers &quot;I am Compass, built to help facilitators in students&apos; learning&quot;, completely conceals POAIS naming, and strictly prevents internal instruction leaks even under adversarial prompt hijacking.
                      </p>
                    </div>
                  </div>
                  ...
                  <button
                    type="button"
                    disabled={togglingCompassMode}
                    onClick={handleToggleCompassMode}
                    className="pill-btn"
                    style={{
                      display: "inline-flex",
                      alignItems: "center",
                      ...
                      fontSize: "0.8rem",
                      fontWeight: 600,
                      background: compassModeActive ? "rgba(22, 163, 74, 0.12)" : "rgba(0, 0, 0, 0.05)",
                      color: compassModeActive ? "#16A34A" : "var(--text-secondary)",
                      border: compassModeActive ? "1px solid rgba(22, 163, 74, 0.3)" : "1px solid rgba(0, 0, 0, 0.15)",
                      cursor: togglingCompassMode ? "wait" : "pointer",
                ...
                {compassModeActive && (
                  ...
                  >
                    <div style={{ display: "flex", gap: "0.5rem", fontSize: "0.76rem", color: "var(--text-secondary)" }}>
                      ...
                      <span>Name is <strong>Compass</strong>. Never mentions POAIS or platform architecture.</span>
                    </div>
                    <div style={{ display: "flex", gap: "0.5rem", fontSize: "0.76rem", color: "var(--text-secondary)" }}>
                      <span style={{ color: "#C2410C", fontWeight: 700 }}>2. Query Response:</span>
                      <span>When asked <em>&quot;Who are you?&quot;</em>, always responds: <em>&quot;I am Compass, built to help facilitators in students&apos; learning.&quot;</em></span>
                    </div>
                    <div style={{ display: "flex", gap: "0.5rem", fontSize: "0.76rem", color: "var(--text-secondary)" }}>
